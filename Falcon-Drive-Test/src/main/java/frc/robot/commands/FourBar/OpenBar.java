/*----------------------------------------------------------------------------*/
/* Copyright (c) 2019 FIRST. All Rights Reserved.                             */
/* Open Source Software - may be modified and shared by FRC teams. The code   */
/* must be accompanied by the FIRST BSD license file in the root directory of */
/* the project.                                                               */
/*----------------------------------------------------------------------------*/

package frc.robot.commands.FourBar;

import edu.wpi.first.wpilibj.Joystick;
import edu.wpi.first.wpilibj2.command.CommandBase;
import edu.wpi.first.wpilibj2.command.RunCommand;
import frc.robot.subsystems.FourBar;

public class OpenBar extends CommandBase {
  /**
   * Creates a new OpenBar.
   */

  public FourBar m_FourBar;

  public OpenBar(final FourBar FourBarInput) {
    // Use addRequirements() here to declare subsystem dependencies.
    m_FourBar = FourBarInput;
    addRequirements(FourBarInput);
  }

  // Called when the command is initially scheduled.
  @Override
  public void initialize() {

  }

  // Called every time the scheduler runs while the command is scheduled.
  @Override
  public void execute() {
    m_FourBar.setPositionMotorMove(0.3);
    m_FourBar.setIntake(-0.5);
  }

  // Called once the command ends or is interrupted.
  @Override
  public void end(final boolean interrupted) {
    m_FourBar.setPositionMotorMove(0.0);
  }

  // Returns true when the command should end.
  @Override
  public boolean isFinished() {
    if(!m_FourBar.getOpenPoition()){
      return false;
    }else{
      return true;
    }
  }
}
